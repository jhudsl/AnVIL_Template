...
if (!is.null(AnVIL_module_settings$notebook_name)) {
    AnVIL_module_settings$text <- paste0('1. Find the Notebook named **', AnVIL_module_settings$notebook_name, '**. Click the "OPEN" button to run and edit this Notebook. (Clicking on its name will open a static preview of the Notebook, which can be useful for viewing your previous work but will not let you run the code.)'
}
```
