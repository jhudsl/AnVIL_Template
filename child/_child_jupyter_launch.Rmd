...
```{r, include=FALSE}
# settings for instructions about which workspace to use
# default
AnVIL_module_settings$text <- "Note that, in order to use Jupyter, **you must have access to a Terra Workspace with permission to compute** (i.e. you must be a **Writer** or **Owner** of the Workspace). If you created or cloned the Workspace yourself, you are good to go! If you are using someone else's Workspace, the Workspace Owner(s) can grant you appropriate permissions, if they haven't already."

# student version
if (AnVIL_module_settings$audience == "student") {
  AnVIL_module_settings$text <- paste0("For this activity, you should launch Jupyter from your copy of the Workspace **", AnVIL_module_settings$workspace_name, "**. (It should be named something like `", AnVIL_module_settings$clone_name, "`). If you don't yet have your own copy, make one now by cloning the Workspace **", AnVIL_module_settings$workspace_name, "** before following the steps below.")
}
```
...
ottrpal::include_slide("https://docs.google.com/presentation/d/1-IU5l40UxmpoBqFimSUmT1PNEUij2UnXMslOsiG5ncE/edit#slide=id.g117989bd49c_0_150")
    ```
```{r, include=FALSE}
# settings for instructions about opening your Workspace
AnVIL_module_settings$text <- "1. Click on the name of your Workspace. You should be routed to a link that looks like:"

# student version - remind students which workspace they should be using
if (AnVIL_module_settings$audience == "student") {
  AnVIL_module_settings$text <- paste0("1. Click on the name of your Workspace (e.g. `", AnVIL_module_settings$clone_name, "`). You should be routed to a link that looks like:")
}
```
...
# Instruct students to open a specific notebook
if (!is.null(AnVIL_module_settings$notebook_name)) {
  AnVIL_module_settings$text <- paste0('1. Find the Notebook named **', AnVIL_module_settings$notebook_name, '**. Click on its name, then click the "OPEN" button to run and edit this Notebook. (Clicking on its name opens a static preview of the Notebook, which can be useful for viewing your previous work but will not let you run the code. You must click the OPEN button to run or edit the code)')
}
```
